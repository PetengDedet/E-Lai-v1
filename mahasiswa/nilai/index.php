		...
		<div class="container" style="">
			<div class="row">
				...
				<div class="col-md-9 col-xs-12" style="background:#fff;padding:20px;">
					...
					<form method="GET" class="form-inline" action="<?php echo $_SERVER['PHP_SELF']; ?>">
						<div class="form-group">
							<select name="dist" class="form-control">
								<option></option>
								<?php
									$qn = mysql_query("SELECT * FROM view_distribusi WHERE ruang_id='$_SESSION[ruang]' AND semester_status='Aktif' AND tahunakademik_status='Aktif'");
									if (mysql_num_rows($qn)>0) {
										while ($n = mysql_fetch_array($qn)) {?>
												<option value="<?php echo $n['distribusi_id']; ?>"><?php echo $n['matakuliah_string']; ?></option>
								<?php										
										}
									}
									else{?>
											<option>Belum ada mata kuliah aktif</option>
								<?php
									}
								?>
							</select>
					...
					<?php
						if (isset($_GET['dist'])) {
							$qnn=mysql_query("SELECT * FROM view_nilai WHERE distribusi_id='$_GET[dist]' AND mahasiswa_npm='$_SESSION[id]' ORDER BY soal_id");
