			...
			<div class="row">
				...
				<div class="col-md-9 col-xs-12" style="background:#fff;padding:20px;">
					...
					<div class="jumbotron well">
						<center><h3>Tahun Akademik aktif saat ini adalah</h3>
								<h1><?php $ak 	= mysql_query("SELECT * FROM tahunakademik WHERE tahunakademik_status='Aktif'");
										  $dak	= mysql_fetch_array($ak);
										  $t = $dak['TAHUNAKADEMIK_ID'];
										  $tt = $dak['TAHUNAKADEMIK_ID']+1;
										  $ttt = $dak['TAHUNAKADEMIK_ID']+2;
										  echo $t." / ".$tt;
								 ?></h1>
								 <br>
								 <form method="POST" action="">
								 <input type="submit" name="aktifkan" class="btn btn-default btn-lg" value="Aktifkan <?php echo $tt.' / '.$ttt; ?>">
								</form>
								<?php 
									if (isset($_POST['aktifkan'])) {
										#echo "<script type='text/javascript'>window.alert('Yakin akan mengaktifkan tahun Akademik?');</script>";
										mysql_query("UPDATE tahunakademik SET tahunakademik_status='Tidak Aktif' WHERE tahunakademik_id='$t'");
										mysql_query("UPDATE tahunakademik SET tahunakademik_status='Aktif' WHERE tahunakademik_id='$tt'");
