							...
							<?php
								...
								if (isset($_POST['kirim'])) {
									...
									if (empty($isikomentar)) {
										...
										mysql_query("INSERT INTO komentar(komentar_id,komentar_isi,komentar_tgl_kirim,komentar_poster_d,topik_id)VALUES('','$isikomentar',NOW(),'$sesidd','$topikid')") or die(mysql_error());
...
elseif ($_SESSION["level"]=="Mahasiswa") {
	...
	if ($jtopik < 1) {
		...
		if ($artopik['ruang_id']==$sesruang) {
			header("location:../../mahasiswa/bahasan/topik.php?t=$tid");
		}
		else{
			header("location:index.php");
		}
