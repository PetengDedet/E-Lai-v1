...
if ($_SESSION["level"]== "Dosen") {
	...
	sambung2();
	$cektopik = mysql_query("SELECT * FROM view_nilai WHERE soal_id='$sid' AND dosen_kode='$_SESSION[id]'");
	$artopik = mysql_fetch_array($cektopik);
	$jtopik = mysql_num_rows($cektopik);
	if ($jtopik < 1) {
		echo "Nilai tidak ditemukan <a href='index.php'>Kembali</a>";
	}
	else{
...
elseif ($_SESSION["level"]=="Mahasiswa") {
	...
	if ($jtopik < 1) {
		...
		if ($artopik['DOSEN_KODE']==$sesruang) {
			header("location:../../mahasiswa/soal/lnilai.php?id=$sid");
		}
		else{
			header("location:index.php");
		}
