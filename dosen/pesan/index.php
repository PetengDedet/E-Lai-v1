...
<!--DOSEN-->
	...
	<!--MOdal				
						...
						<div class="form-group">
  						  ...
  						  <div class="col-sm-10">
  						    <select name="dosen" class="form-control input-sm">
								<?php 
									#$ruang = $_SESSION['id'];
									#$caridosen = mysql_query("SELECT * FROM view_distribusi WHERE dosen_kode='$ruang'");
		...
		<?php 
						...
						#}
					?>
					<button class="btn btn-primary" data-toggle="modal" data-target=".bs-example-modal-lg"><span class="glyphicon glyphicon-envelope"></span> Pesan Baru</button>
					-->
					<?php 
						#baris per halaman
						$bph = 10;
						...
						$offset = ($halke-1)*$bph;
						$sesid = $_SESSION['id'];
						$qtopik = mysql_query("SELECT pesan.*,mahasiswa.mahasiswa_nama FROM pesan LEFT JOIN mahasiswa ON pesan.mahasiswa_npm=mahasiswa.mahasiswa_npm WHERE dosen_kode='$sesid' ORDER BY pesan_tgl_kirim DESC LIMIT $offset,$bph");
						...
						?>
								...
								<tr>
									...
									<td><?php echo $htopik['mahasiswa_nama']; ?></td>
						...
						$hitungtopik = mysql_query("SELECT COUNT(*) AS jtotaltopik FROM pesan WHERE dosen_kode='$_SESSION[id]'");
